...
<div class="mb-4 md:mb-6">
    <a href="{{ route('petugas.pengaduan.show', $pengaduan) }}" 
       class="inline-flex items-center px-3 md:px-4 py-2 md:py-2.5 bg-white border border-slate-300 rounded-lg shadow-sm text-xs md:text-sm text-slate-700 hover:bg-slate-50 transition-all duration-200 font-medium">
        <i class="fas fa-arrow-left mr-2 text-xs md:text-sm"></i>
...
<div class="max-w-3xl mx-auto px-0">
    <div class="bg-white rounded-lg md:rounded-xl shadow-sm overflow-hidden hover:shadow-lg transition-shadow duration-300">
        ...
        <form action="{{ route('petugas.item-request.store', $pengaduan) }}" method="POST" enctype="multipart/form-data" class="p-4 md:p-6">
            ...
            <!-- Error Messages -->
            @if ($errors->any())
            <div class="mb-4 md:mb-6 bg-red-50 border-l-4 border-red-400 p-3 md:p-4 rounded-r-lg">
                <div class="flex items-start">
                    <i class="fas fa-exclamation-circle text-red-400 mr-2 md:mr-3 text-base md:text-lg mt-0.5 flex-shrink-0"></i>
                    <div class="flex-1">
                        <h3 class="text-xs md:text-sm font-bold text-red-800">Terdapat kesalahan:</h3>
            ...
            <div class="space-y-4 md:space-y-6">
                <!-- Nama Barang -->
                <div>
                    <label for="nama_barang_baru" class="block text-xs md:text-sm font-bold text-slate-700 mb-2 md:mb-3">
                ...
                <!-- Lokasi (Read Only) -->
                <div>
                    <label for="lokasi_barang_baru" class="block text-xs md:text-sm font-bold text-slate-700 mb-2 md:mb-3">
                    ...
                    <div class="relative">
                        <input type="text" 
                               ...
                               class="w-full px-3 md:px-4 py-2 md:py-3 text-xs md:text-base bg-slate-100 border-2 border-slate-300 rounded-lg shadow-sm cursor-not-allowed"
                               readonly
                               required>
                        <div class="absolute right-3 md:right-4 top-1/2 -translate-y-1/2">
                            <i class="fas fa-lock text-slate-400 text-xs md:text-sm"></i>
                        </div>
                    </div>
                    <p class="text-xs text-slate-500 mt-1.5 md:mt-2 flex items-center">
                ...
                <!-- Alasan Permintaan -->
                <div>
                    <label for="alasan_permintaan" class="block text-xs md:text-sm font-bold text-slate-700 mb-2 md:mb-3">
                ...
                <div>
                    ...
                    <div id="imagePreview" class="mt-4 hidden">
                        <div class="relative inline-block">
                            <img src="/placeholder.svg" alt="Preview" class="max-w-full md:max-w-sm rounded-lg shadow-md mx-auto">
                            <button type="button" 
                                    onclick="removeImage()" 
                ...
                <div class="p-3 md:p-4 bg-blue-50 rounded-lg border-l-4 border-blue-500">
                    <div class="flex items-start">
                        <i class="fas fa-info-circle text-blue-500 mt-0.5 mr-2 md:mr-3 text-sm md:text-base flex-shrink-0"></i>
                        <div class="flex-1">
                ...
                <!-- Action Buttons -->
                <div class="flex flex-col sm:flex-row justify-end gap-2 md:gap-3 pt-4 md:pt-6 border-t border-slate-200">
                    <a href="{{ route('petugas.pengaduan.show', $pengaduan) }}"
                       class="w-full sm:w-auto px-4 md:px-6 py-2 md:py-3 bg-slate-100 text-slate-700 text-xs md:text-sm rounded-lg font-semibold hover:bg-slate-200 transition-colors text-center">
                        Batal
                    </a>
                    <button type="submit"
